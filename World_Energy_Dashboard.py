...
st.title(':abacus: World Energy Dashboard')
st.markdown('<style>div,block-container{padding-top:0rem;}<style>', unsafe_allow_html = True)


# 02 IMPORTING DATASET

dataset_path = 'https://raw.githubusercontent.com/azzindani/00_Data_Source/main/World_Energy.csv'
df = pd.read_csv(dataset_path)
df.columns = df.columns.str.replace('_', ' ', regex=True)
df.columns = df.columns.str.replace('pct', '(%)', regex=True)
df.columns = df.columns.str.replace('twh', '(Tera Watt Hour)', regex=True)
...
colors_1 = px.colors.sequential.Oryel
colors_2 = px.colors.sequential.Oryel_r
